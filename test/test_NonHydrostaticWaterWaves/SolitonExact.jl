...
#     end
    function utheorfun(t::Float64, x::Vector{Matrix{Float64}}) 
        ...
        h, q1, q2, q3, p        = SolitonExactFun(t, x[1], A=A, gamma=3/2, 
                                                h0=h0, x0=0.0, g=g)
        b                       = @. 0.0*x[1]
        eta                     = h
...
#     TrMesh_Rectangle_Create!(MeshFile, -5.0, 5.0, NX, -5.0, 5.0, NY)
    ...
    solver.ProblemName      = "SolitonExact" 
    solver.SC               = SC
...
#             x12     = xend
    ...
    function SaveSol()
        ...
        if SaveRes && ( solver.t-t_lastRes>=Deltat_SaveRes ||
            ...
            save("$(ResUbi)LIRKHyp_SC$(SC)_$(nb_SaveRes).jld2", "StudyCase", "SolitonExact",
...
function SolitonExactFun(t::Float64, x::AMF64; A::Float64=0.2, gamma::Float64=3/2, 
