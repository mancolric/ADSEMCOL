...
function Gaussian(hp0::Float64, FesOrder::Int;
    ...
    function u0fun(x::Vector{Matrix{Float64}})
        ...
        h               = @tturbo @. h0 + A*exp(-(x[1]/1.0)^2)
        q1              = @tturbo @. 0.0*x[1]
        q2              = @tturbo @. 0.0*x[1]
        q3              = @tturbo @. 0.0*x[1]
        P               = @tturbo @. h*h
        b               = @tturbo @. 0.0*x[1]
        return [h+b, q1, q2, q3, P, b]
...
#     BC_right        = DoNothing1()
    ...
    #Mesh:
    MeshFile                = "../temp/Gaussian$(SC).geo"
    NX                      = Int(ceil((xend+20.0)/(hp0*FesOrder)))
    NY                      = 2
    TrMesh_Rectangle_Create!(MeshFile, -20.0, 20.0, NX, -5.0, 5.0, NY)
...
#     return
    ...
    function PlotSol()
        ...
        if PlotFig && ( solver.t-t_lastFig>=Deltat_SaveFig ||
            ...
            if SaveFig
                savefig("$(VideosUbi)Gaussian$(SC)_$(nb_SaveFig).png", dpi=400, pad_inches=0)
            end
            ...
            if SaveFig
                savefig("$(VideosUbi)GaussianNodes$(SC)_$(nb_SaveFig).png", dpi=400, pad_inches=0)
            end
    ...
    function SaveSol()
        ...
        if SaveRes && ( solver.t-t_lastRes>=Deltat_SaveRes ||
            ...
            save("$(ResUbi)LIRKHyp_SC$(SC)_$(nb_SaveRes).jld2", "StudyCase", "Gaussian",
    ...
    #Save results:
    if SaveRes
        save("$(ResUbi)LIRKHyp_SC$(SC)_1000.jld2", "StudyCase", "Gaussian",
