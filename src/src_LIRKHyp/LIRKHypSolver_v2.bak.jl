...
#     solver.Integ2D      = TrInt(solver.mesh, solver.AMA_ProjOrder, N=solver.AMA_ProjN)
#     @warn "Integ2D"
...
#     solver.fNStab_RK    = zeros(solver.nVars*solver.res_fes.nDof, solver.RK.stages)
#     println("f and Ju = ", time()-t_ini)
...
#     urec,       = AMA_CD_PatchReconstruction(uP, solver.fes.PSpace, solver.rec_fes)
#     println("Reconstruction performed in ", time()-t_ini, " seconds")
#     urec,       = AMA_BC_PatchReconstruction(u, solver.fes, solver.rec_fes)
...
#         erec                    = Vector{Vector{Float64}}(undef,solver.nVars)
        @inbounds for II=1:solver.nVars
            erec[II]            = zeros(size(urec[II]))
            #Add urec:
            for iElem=1:solver.mesh.nElems, iDof=1:solver.rec_fes.DofPerElem
                dof             = (iElem-1)*solver.rec_fes.DofPerElem + iDof
                erec[II][dof]   = urec[II][dof]
            end
            #Substract u:
            uII                 = uP[II][solver.fes.PSpace.ElemsDof]*transpose(solver.fes.PSpace.Lag_PKD)
            for iElem=1:solver.mesh.nElems, iDof=1:solver.fes.PSpace.DofPerElem
                dof             = (iElem-1)*solver.rec_fes.DofPerElem + iDof
                erec[II][dof]   -= uII[iElem,iDof]
            end
...
function L2Projection!(solver::SolverData, u0::Vector{Float64}, u0_fes::TrPBSpace)
    #(This function is used by post-time-step approach)
...
function LIRKHyp_Step_Post_CB!(solver::SolverData)
...
#             solver.tetaS                    += time()-t_ini
            solver.tAMA                     += time()-t_ini
#             global tAMA3                    += time()-t_ini
            #(in the pre- approach, the CPU cost of the reconstruction is computed inside
            #AdaptMesh!.)
...
#                         AMA_TolS[kk]    = solver.SfS*solver.TolS_max
                    ...
                    else    #otherwise, leave TolS_kk as is #NOT POSSIBLE
                    end
...
#                     metric      = AMA_MetricIntersection(AMA_metric[[1,solver.RK.stages]])
                    metric      = AMA_metric[solver.RK.stages]
...
#     return LIRKHyp_Step_Post_CB!(solver)
