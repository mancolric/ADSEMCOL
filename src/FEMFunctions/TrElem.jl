...
function MassMatrixElem(FES::TrFES)

    #Create quadrature rule:
    QRule       = WVRule(FES.order)
    w_qp        = diagm(QRule.w)
    Nm_qp       = NCompute(FES, QRule.xi)
    Mm          = transpose(Nm_qp)*w_qp*Nm_qp
    return Mm
    
end

function GetDominantTerms(Am::Matrix{Float64})
    ...
    for ii=1:m
        ...
        #Sort rows:
        cols        = sortperm(Am[ii,:], rev=true)
        vals        = Am[ii,cols]
        
        #Compute sum of terms:
        cumsumv     = cumsum(vals)
        threshold   = 0.5*cumsumv[n]
        
        #Retain terms such that cumulative sum is larger than threshold:
        T           = findfirst(cumsumv.>=threshold)
        append!(iv, fill(ii, T))
        append!(jv, cols[1:T])
